...
public class BestPeersService : NSObject {
    ...
    let task = URLSession.shared.dataTask(with: url) { [self] (data, response, error) in
        ...
        guard let data = data else {
            completionHandler(BestPeersResult.Error(message: "No data received"))
            return
        }

        do {
            let htmlData = String(data: data, encoding: .ascii)!

            let pattern = "((tcp|tls):\\/\\/(.*?))<\\/"
            let regex = try NSRegularExpression(pattern: pattern)

            var fastPeers = 0
            var peers = [PeerInfo]()  // Make sure to capture self explicitly
