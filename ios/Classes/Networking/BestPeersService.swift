...
public class BestPeersService: NSObject {
    ...
    public func GetBestPeers(completionHandler: @escaping (BestPeersResult) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            guard let data = data, let htmlData = String(data: data, encoding: .ascii) else {
                completionHandler(BestPeersResult.Error(message: "Failed to parse HTML data"))
                return
            }
            ...
            do {
                ...
                completionHandler(BestPeersResult.Success(peers: self.peers))
            } catch {
                completionHandler(BestPeersResult.Error(message: error.localizedDescription))
            }
        }
